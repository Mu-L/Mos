...
class KeyRecorder: NSObject {
    ...
    func startRecording(from sourceView: NSView, mode: KeyRecordingMode = .combination) {
        ...
        DispatchQueue.main.async {
            LogiSessionManager.shared.temporarilyDivertAll()
        }
        // 监听事件
        do {
            // 监听回调事件通知
            ...
            // 监听 HID++ 事件 (如果 LogiSessionManager 已启动)
            hidEventObserver = NotificationCenter.default.addObserver(
                forName: NSNotification.Name("LogiButtonEvent"),
                object: nil,
                queue: .main
    ...
    func stopRecording() {
        ...
        LogiSessionManager.shared.restoreDivertToBindings()
