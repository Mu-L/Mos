...
final class LogitechConflictDetectorTests: XCTestCase {
    ...
    func testStatus_notQueried_returnsUnknown() {
        let status = LogitechConflictDetector.status(
            reportingFlags: 0x01,
            targetCID: 0,
            cid: cid,
    ...
    func testStatus_unchangedByMosOperations() {
        ...
        let firstStatus = LogitechConflictDetector.status(
        ...
        let afterUndivertStatus = LogitechConflictDetector.status(
            reportingFlags: 0x01, targetCID: 0, cid: cid, reportingQueried: true
        )
